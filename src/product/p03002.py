...
from featureparser.parser_bus_variables import ParserBusVariables
...
class P03002(Generate):
    ...
    def strategy_process(self):
        ...
        try:
            ...
            # 处理解析落库，初筛报告需要传reportReqNo号和产品编号
            parse_resp = resp_end.copy()
            report_req_no = strategy_param.get('reportReqNo')
            pbv = ParserBusVariables(parse_resp, report_req_no, product_code)
            pbv.process()
    ...
    def _strategy_hand(self, json_data, data, product_code, req_no):
        ...
        origin_input['segment_name'] = 'rule'
        if base_type.endswith("OTHER"):
            origin_input['strategy'] = "02"
        else:
            origin_input['strategy'] = strategy
    ...
    def _get_strategy_second_array(self, data, fundratio, relation, strategy_resp, user_name, user_type,
        ...
        array['td_pred_new'] = self._get_json_path_value(strategy_resp, '$..td_pred_new')
    ...
    def _shake_hand_response(self, base_type_service, data, product_code, req_no):
        ...
        variables['segment_name'] = 'HAND_SHAKE'
        # base_type 以OTHER结尾,strategy="02"
        if base_type.endswith("OTHER"):
            variables['strategy'] = "02"
        ...
        #更新strategy
        if base_type.endswith("OTHER"):
            data.get("extraParam")['strategy'] = "02"
        resp.update(data)
