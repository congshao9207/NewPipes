...
class T16002(Transformer):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.variables = {
            'court_ent_admi_vio': 0,  # 命中行政违法,需调查反馈
            'court_ent_judge': 0,  # 命中民商事裁判,需调查反馈
            'court_ent_trial_proc': 0,  # 命中民商事审判,需调查反馈
            'court_ent_tax_pay': 0,  # 命中纳税非正常户,需调查反馈
            'court_ent_owed_owe': 0,  # 命中欠款欠费
            'court_ent_tax_arrears': 0,  # 命中欠税名单,需调查反馈
            'court_ent_dishonesty': 0,  # 命中法院失信名单
            'court_ent_limit_entry': 0,  # 命中限制出入境名单
            'court_ent_high_cons': 0,  # 命中限制高消费名单
            'court_ent_pub_info': 0,  # 命中法院执行名单,需调查反馈
            'court_ent_cri_sus': 0,  # 命中犯罪及嫌疑人
            'court_ent_tax_arrears_amt_3y': 0,  # 三年内欠税总金额大,需调查反馈
            'court_ent_pub_info_amt_3y': 0,  # 三年内执行公开涉案金额大,需调查反馈
            'court_ent_admi_vio_amt_3y': 0,  # 三年内行政违法涉案金额大,需调查反馈
            'court_ent_judge_amt_3y': 0,  # 三年内民商事裁判涉案金额大,需调查反馈
            'court_ent_docu_status': 0,
            'court_ent_proc_status': 0,
            'court_ent_fin_loan_con': 0,  # 命中金融借款合同纠纷
            'court_ent_loan_con': 0,  # 命中借款合同纠纷
            'court_ent_pop_loan': 0,  # 命中民间借贷纠纷
            ...
            'court_ent_max_execute_amt': 0  # 新增：命中大额执行案件
        }
    ...
    def _ps_court_excute_public(self, df=None):
        if df is not None and len(df) > 0:
            ...
            if df_max is not None and len(df_max) > 0:
                ...
                self.variables['court_ent_max_execute_amt'] = float("%.2f" % df_max['max_money'].max())
